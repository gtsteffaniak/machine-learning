...
parser.add_argument("--jobID", type=str, default="999", help='speficy jobid for multiple executions')
args = parser.parse_args()
randomID=str(randint(1000, 5000))
print(args)
...
def extract_frames(video, outDir):
    ...
    print('{} -i {} -vsync 0 -q:v 1 {}/%06d.jpg'.format(ffmpeg_path, video, outDir))
    retn = os.system('{} -i "{}" -vsync 0 -q:v 1 {}/%06d.jpg'.format(ffmpeg_path, video, outDir))
    if retn:
...
def create_video(dir):
    ...
    error = ""
    print('{} -r {} -i {}/%d.jpg -vcodec libx264 {}'.format(ffmpeg_path, args.fps, dir, args.output))
    retn = os.system('{} -r {} -i {}/%d.jpg -vcodec libx264 "{}"'.format(ffmpeg_path, args.fps, dir, args.output))
    if retn:
...
def main():
    ...
    if not os.path.isdir("C:\\temp"):
        os.mkdir("C:\\temp")
    IS_WINDOWS = 'Windows' == platform.system()
    extractionDir = "C:\\temp\\tmpSuperSloMo"+randomID
    ...
    dict1 = torch.load(args.checkpoint, map_location='cuda:0')
    ...
    with torch.no_grad():
        for _, (frame0, frame1) in enumerate(tqdm(videoFramesloader), 0):
            ...
            for batchIndex in range(args.batch_size):
                (TP(frame0[batchIndex].to(device).detach())).resize(videoFrames.origDim, Image.Resampling.BILINEAR).save(os.path.join(outputPath, str(frameCounter + args.sf * batchIndex) + ".jpg"))
            frameCounter += 1
            ...
            for intermediateIndex in range(1, args.sf):
                ...
                for batchIndex in range(args.batch_size):
                    (TP(Ft_p[batchIndex].to(device).detach())).resize(videoFrames.origDim, Image.Resampling.BILINEAR).save(os.path.join(outputPath, str(frameCounter + args.sf * batchIndex) + ".jpg"))
                    #framedata.cat((TP(Ft_p[batchIndex].to("cuda:0").detach())).resize(videoFrames.origDim, Image.Resampling.BILINEAR))
                frameCounter += 1
...
def mainMulit():
    ...
    for i in range(worker):
        p = mp.Process(target=run, args=(t,))
        p.daemon=True	
        jobs.append(p)
        p.start()
